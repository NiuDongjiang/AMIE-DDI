...
class Graph_Bert_Dataset_fine_tune(object):
    ...
    def tf_numerical_seq_(self,smiles1,smiles2,labels):
    ...
    def tf_numerical_seq(self, smiles1, smiles2, labels):
        # 获取 motif 级别和原子级别的特征
        x1, x2, adjoin_matrix1, adjoin_matrix2, dist_matrix1, dist_matrix2, \
            atom_features1, atom_features2, adjoin_matrix1_atom, adjoin_matrix2_atom, \
            dist_matrix1_atom, dist_matrix2_atom, atom_match_matrix1, atom_match_matrix2, \
            sum_atoms1, sum_atoms2, y = tf.py_function(
            self.numerical_seq,
            [smiles1, smiles2, labels],
            [tf.int64, tf.int64, tf.float32, tf.float32, tf.float32, tf.float32,
             tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32,
             tf.float32, tf.float32, tf.float32, tf.float32, tf.int64])

        # 获取 motif 节点数
        num_motif_nodes1 = tf.shape(x1)[0]
        num_motif_nodes2 = tf.shape(x2)[0]

        # 找到当前药物对中的最大 motif 节点数
        max_motif_nodes = tf.maximum(num_motif_nodes1, num_motif_nodes2)

        # 确保 max_motif_nodes >= num_motif_nodes1 和 max_motif_nodes >= num_motif_nodes2
        pad_motif_nodes1 = tf.maximum(max_motif_nodes - num_motif_nodes1, 0)
        pad_motif_nodes2 = tf.maximum(max_motif_nodes - num_motif_nodes2, 0)

        # 填充 motif 节点数
        x1 = tf.pad(x1, [[0, pad_motif_nodes1]])
        x2 = tf.pad(x2, [[0, pad_motif_nodes2]])

        adjoin_matrix1 = tf.pad(adjoin_matrix1, [[0, pad_motif_nodes1], [0, pad_motif_nodes1]], constant_values=0)
        adjoin_matrix2 = tf.pad(adjoin_matrix2, [[0, pad_motif_nodes2], [0, pad_motif_nodes2]], constant_values=0)

        dist_matrix1 = tf.pad(dist_matrix1, [[0, pad_motif_nodes1], [0, pad_motif_nodes1]], constant_values=-1e9)
        dist_matrix2 = tf.pad(dist_matrix2, [[0, pad_motif_nodes2], [0, pad_motif_nodes2]], constant_values=-1e9)

        # 设置 motif 节点的形状，保持动态部分为 None
        x1.set_shape([None])
        x2.set_shape([None])
        adjoin_matrix1.set_shape([None, None])
        adjoin_matrix2.set_shape([None, None])
        ...
        # 获取原子级节点数
        num_atoms1 = tf.shape(atom_features1)[0]
        num_atoms2 = tf.shape(atom_features2)[0]

        # 找到当前药物对中的最大原子节点数
        max_atoms = tf.maximum(num_atoms1, num_atoms2)

        # 确保 max_atoms >= num_atoms1 和 max_atoms >= num_atoms2
        pad_atoms1 = tf.maximum(max_atoms - num_atoms1, 0)
        pad_atoms2 = tf.maximum(max_atoms - num_atoms2, 0)

        # 填充原子级节点数
        atom_features1 = tf.pad(atom_features1, [[0, pad_atoms1], [0, 0]])
        atom_features2 = tf.pad(atom_features2, [[0, pad_atoms2], [0, 0]])

        adjoin_matrix1_atom = tf.pad(adjoin_matrix1_atom, [[0, pad_atoms1], [0, pad_atoms1]], constant_values=0)
        adjoin_matrix2_atom = tf.pad(adjoin_matrix2_atom, [[0, pad_atoms2], [0, pad_atoms2]], constant_values=0)

        dist_matrix1_atom = tf.pad(dist_matrix1_atom, [[0, pad_atoms1], [0, pad_atoms1]], constant_values=-1e9)
        dist_matrix2_atom = tf.pad(dist_matrix2_atom, [[0, pad_atoms2], [0, pad_atoms2]], constant_values=-1e9)

        # 确保 sum_atoms1 和 sum_atoms2 的节点数是相同的，均为 max_motif_nodes - 1
        max_sum_atoms_nodes = max_motif_nodes - 1
        pad_sum_atoms1 = tf.maximum(max_sum_atoms_nodes - tf.shape(sum_atoms1)[0], 0)
        pad_sum_atoms2 = tf.maximum(max_sum_atoms_nodes - tf.shape(sum_atoms2)[0], 0)

        atom_match_matrix1 = tf.pad(atom_match_matrix1, [[0, pad_sum_atoms1], [0, pad_atoms1]], constant_values=0)
        atom_match_matrix2 = tf.pad(atom_match_matrix2, [[0, pad_sum_atoms2], [0, pad_atoms2]], constant_values=0)

        sum_atoms1 = tf.pad(sum_atoms1, [[0, pad_sum_atoms1], [0, 0]], constant_values=1)
        sum_atoms2 = tf.pad(sum_atoms2, [[0, pad_sum_atoms2], [0, 0]], constant_values=1)

        # 设置原子级节点的形状，保持动态部分为 None
        atom_features1.set_shape([None, None])
        atom_features2.set_shape([None, None])
        adjoin_matrix1_atom.set_shape([None, None])
        adjoin_matrix2_atom.set_shape([None, None])
