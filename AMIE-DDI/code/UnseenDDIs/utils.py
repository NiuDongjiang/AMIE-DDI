...
class Mol_Tokenizer():
    def __init__(self,tokens_id_file):#通过传入的 tokens_id_file 文件加载一个分子片段的词汇表（vocab），用于将分子的不同部分（片段）转换成唯一的标记（ID）。
    ...
    def tokenize(self,smiles):
        ...
        ids,edge = self.tree_decomp(mol) #调用 tree_decomp 方法对分子进行分解，获取分子片段的标识符（ids）和边（edge）。
        motif_list = []
        for id_ in ids:
            _,token_mols = self.get_clique_mol(mol,id_)#遍历这些标识符，对每个标识符调用 get_clique_mol 方法获取对应的分子片段，然后根据词汇表获取该片段的标记 ID。如果片段不在词汇表中，则使用一个特殊的 <unk> 表示。
    ...
    def sanitize(self,mol):#用于标准化分子对象，确保生成合法的 SMILES 表示，并可以处理异常情况。
        try:
            ...
            return None
        return mol
    def get_mol(self,smiles):#通过 SMILES 字符串生成分子对象。
        mol = Chem.MolFromSmiles(smiles)
        if mol is None:
            return None
        Chem.Kekulize(mol)
        return mol
    def get_smiles(self,mol):#将分子对象转换为 SMILES 字符串，支持 Kekulé 结构。
        return Chem.MolToSmiles(mol, kekuleSmiles=True)
    def get_clique_mol(self,mol,atoms_ids):#通过给定的原子 ID 列表提取分子中的片段，返回该片段的分子表示和对应的 SMILES 字符串。这里使用了 RDKit 的 MolFragmentToSmiles 函数来提取特定原子的片段。
