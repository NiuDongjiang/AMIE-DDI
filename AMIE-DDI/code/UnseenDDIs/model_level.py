...
class BANLayer(tf.keras.layers.Layer):
    def __init__(self, v_dim, q_dim, h_dim, h_out, act='relu', dropout=0.2, k=3):
        ...
        self.v_net = FCNet([v_dim, h_dim*2], act=act, dropout=dropout)#FCNet([v_dim, h_dim*k], act=act, dropout=dropout)  [v_dim, h_dim*2]
        self.q_net = FCNet([q_dim, h_dim*2], act=act, dropout=dropout)#FCNet([q_dim, h_dim*k], act=act, dropout=dropout)  [v_dim, h_dim*2]
    ...
    def build(self, input_shape):
        ...
        if self.h_out <= self.c:
            self.h_mat = self.add_weight(
                shape=(1, self.h_out, 1, self.h_dim*2),#self.h_dim*2
...
class EncoderLayer(keras.layers.Layer):
    ...
    def call(self, x, training, encoder_padding_mask,adjoin_matrix,dist_matrix):
        ...
        #attn_output, att_map = self.bcn(x_l, x_g)
...
class EncoderModel_motif(keras.layers.Layer):
    def __init__(self, num_layers, input_vocab_size,
        ...
        self.bcn = BANLayer(v_dim=int(d_model), q_dim=int(d_model), h_dim=int(d_model / 2), h_out=2)
    def call(self, x, training,atom_level_features,adjoin_matrix = None,dist_matrix = None):
        ...
        x_temp, att = self.bcn(x[:,1:,:], atom_level_features)
        ...
        for i in range(self.num_layers):
            ...
            #x_list.append(x)
        #x, att = self.bcn(x_list[0], x_list[-1])
...
class Co_Attention_Layer(keras.layers.Layer):
    ...
    def call(self, inputs):
        #print(f"\nUpdated temperature to {self.temperature:.4f}")
